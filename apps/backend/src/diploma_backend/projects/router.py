...
_GENERATION_SYSTEM_PROMPT = (
    ...
    "relevant claims in them and cite each one in-text as (Author, Year, \"Title\") — using the "
    "source's own title/year in place of the author when no author name is given — when you draw "
    "on it directly. If the excerpt you are citing is prefixed with a \"[стр. N]\" page marker, "
    "include that page number too, formatted as (Author, Year, \"Title\", с. N); when no such "
    "marker is present, omit the page number entirely rather than guessing one. An excerpt marked "
    ...
    "invent a citation or a page number for a source that was not provided; if none of the "
    "provided sources are relevant to a claim, state it plainly without a citation rather than "
    "fabricating one. Output ONLY the chapter's own body text — cite sources in-text as they come "
    "up, but never append a \"Список использованных источников\"/\"References\"/\"Bibliography\" "
    "heading or list of any kind at the end; the thesis's bibliography lives in its own separate "
    "section elsewhere in the document, not inside this chapter."
)
...
_ANCHOR_GENERATION_SYSTEM_PROMPT = (
    ...
    "\"Title\") — using the source's own title/year in place of the author when no author name is "
    "given — when you draw on it directly. If the excerpt you are citing is prefixed with a "
    "\"[стр. N]\" page marker, include that page number too, formatted as (Author, Year, "
    "\"Title\", с. N); when no such marker is present, omit the page number entirely rather than "
    "guessing one. An excerpt marked \"[REQUIRED]\" is a source the student specifically asked to "
    ...
    "resort, not a default. Never invent a citation or a page number for a source that was not "
    "provided; if none of the provided sources are relevant to a claim, state it plainly without "
    "a citation rather than fabricating one. Output ONLY the new text to insert — cite sources "
    "in-text as they come up, but never append a "
    "\"Список использованных источников\"/\"References\"/\"Bibliography\" "
