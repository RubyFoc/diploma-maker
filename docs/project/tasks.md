...
| TASK-E16-2 | `POST /chapters/{id}/undo`, `/redo` endpoints: replay/revert `Operation` rows against the current draft; reject with a clear error if an op's anchor block no longer exists | E16 | python-developer | done |
| TASK-E16-3 | Redo-stack wipe on a new edit applied after an undo (linear op-log, no branching history) | E16 | python-developer | done |
| TASK-E16-4 | Client-side page-range revert resolution: `PaginatedDocument`'s per-page block indices resolved into a block-id range, sent to the backend as a batch-undo | E16 | frontend-developer | done |
| TASK-E16-5 | Undo/redo UI controls at whole-document/page-range/paragraph-or-line granularity | E16 | frontend-developer | done |
